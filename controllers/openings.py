...
def similar(body):
    try:
        ...
        return {"recommendations": [df.iloc[i[0]].id for i in opening_objs]}
    except:
...
def get_application_score(body, request):
    try:
        ...
        opening_description_topic_embeddings = get_embeddings(
            opening_description_topics, model, tokenizer
        )
        organization_values_topic_embeddings = get_embeddings(
            organization_values_topics, model, tokenizer
        )

        profile_similarity_score = get_emb_score(
            profile_topic_embeddings,
            opening_description_topic_embeddings,
            organization_values_topic_embeddings,
        )
        if is_resume_included:
            resume_similarity_score = get_emb_score(
                resume_topic_embeddings,
                opening_description_topic_embeddings,
                organization_values_topic_embeddings,
            )
        else:
        ...
        if is_resume_included:
            if (
                profile_score > dynamic_max_profile_score * 0.1
                and resume_score > dynamic_max_resume_score * 0.1
            ):
        ...
        overall_fit_score = (
            ...
            + weighted_work_ex_score
        )

        max_score = (
            1 * weight_sentiment
            + profile_score_cap * weight_profile
            + resume_score_cap * weight_resume
            + 3 * weight_work_ex
        )  # Maximum possible weighted score
        min_score = -0.1  # Minimum possible weighted score

        overall_fit_score_normalized = (overall_fit_score - min_score) / (
            max_score - min_score
        )

        return {"score": overall_fit_score_normalized}
...
def get_emb_score(
    ...
    for i in embeddings:
        for j in np.concatenate(
            (opening_description_topic_embeddings, organization_values_topic_embeddings)
        ):
